...
const env = { ...process.env, PUBLIC_MIRA_URL: 'https://mira.maccrate.ai/', PUBLIC_RELEASE_ID: 'release-test' };
...
check(combined.includes('https://mira.maccrate.ai/'), 'production Mira URL is missing');
...
check(/<h1[^>]*>[\s\S]*Mira[\s\S]*Machine/i.test(home), 'homepage has no Mira Machine H1');
