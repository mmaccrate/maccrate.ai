...
const env = { ...process.env, PUBLIC_MIRA_URL: '/mira/', PUBLIC_CARTRIDGES_URL: '/cartridges/', PUBLIC_RELEASE_ID: 'release-test' };
execFileSync('npm', ['run', 'build'], { cwd: root, env, stdio: 'inherit' });

const check = (value, message) => { if (!value) throw new Error(message); };
const file = path => readFileSync(resolve(root, 'dist', path), 'utf8');
const expected = ['index.html','about/index.html','privacy/index.html','terms/index.html','projects/mira-machine/index.html','projects/hello-world-ai-fine-tuning/index.html','projects/browser-lora-cartridges/index.html','404.html','sitemap.xml','og.png'];
...
const browserLora = file('projects/browser-lora-cartridges/index.html');
...
check(/<h1[^>]*>[\s\S]*LoRA,[\s\S]*On Demand/i.test(home), 'homepage does not lead with the newest project');
...
check(fineTuning.includes('WHAT HAPPENED') && fineTuning.includes('57 of 64'), 'fine-tuning field report is missing its experiment and result');
check(browserLora.includes('I made LoRA adapters') && browserLora.includes('hot-swappable'), 'browser LoRA article is missing its technical thesis');
check(browserLora.includes('same request') && browserLora.includes('base response stays visible'), 'browser LoRA article is missing its fair comparison contract');
check(browserLora.includes('wllama-lora') && browserLora.includes('without reloading the base'), 'browser LoRA article is missing its runtime boundary');
check(!/Chordbound|Visit Prep/i.test(browserLora), 'retired cartridge experiments remain in the current article');
...
for (const route of ['https://maccrate.ai/','https://maccrate.ai/about','https://maccrate.ai/projects/mira-machine','https://maccrate.ai/projects/browser-lora-cartridges','https://maccrate.ai/privacy']) check(sitemap.includes(route), `sitemap missing ${route}`);
